			...
			public override void Run ()
			{
				...
				while (true) {
					try {
						byte cur;
						string outStr = ""; 
						while(0 != (cur = System.Convert.ToByte((mmInStream.ReadByte())))){
							string last = System.Text.Encoding.UTF8.GetString(new byte[]{cur});
							outStr = outStr + last;
							if (outStr.Contains("\r\n"))
							{
						...
						_handler.ObtainMessage (BluetoothChat.MESSAGE_READ, outStr.Length, -1, outStr).SendToTarget ();
					} catch (Java.IO.IOException e) {
